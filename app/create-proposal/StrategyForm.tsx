...
const StrategyForm = ({ setActiveStep }: OwnProps) => {
    ...
    const initialErc721Entries: ERC721Entry[] = initialStrategies
      ...
      .map((strategy) => {
        ...
        return params.tokens.map((token, index) => ({ token, weight: params.weights[index] }));
      })
      .flat();
    const initialWhitelistEntries: WhitelistEntry[] = initialStrategies
      ...
      .map((strategy) => {
        ...
        return Object.keys(params.addresses).map((address) => ({ address, weight: params.addresses[address] }));
      })
      .flat();

    const [strategies, setStrategies] = useState<Strategy[]>(initialStrategies);
    const [erc721Entries, setErc721Entries] = useState<ERC721Entry[]>(initialErc721Entries);
    ...
    const handleClick = () => {
        ...
        if (erc721Entries.length > 0) {
          const erc721Index = strategies.findIndex(s => s.name === StrategyName.ERC721);
          const existingStrategy = strategies[erc721Index];
          updatedStrategies[erc721Index] = {
            ...existingStrategy,
            params: {
              symbol: (existingStrategy.params as Erc721MultiRegistryWeightedStrategy['params']).symbol,
        ...
        if (whitelistEntries.length > 0) {
          const whitelistIndex = strategies.findIndex(s => s.name === StrategyName.WHITELIST);
    ...
    const formValidation = () => {
        for (const strategy of strategies) {
          if (strategy.name === StrategyName.ERC721) {
            const params = strategy.params as Erc721MultiRegistryWeightedStrategy['params'];
            if (!params.symbol || params.tokens.some(token => !isValidAddress(token))) {
              return true;
            }
    ...
    const handleSymbolChange = (index: number, value: string) => {
            ...
            (updatedStrategies[index].params as Erc721MultiRegistryWeightedStrategy['params']).symbol = value;
    ...
    const handleErc721EntryChange = (
        ...
        updatedErc721Entries[index] = { ...updatedErc721Entries[index], [key]: value };
    ...
    const handleWhitelistChange = (
        ...
        updatedWhitelistEntries[index] = { ...updatedWhitelistEntries[index], [key]: value };
      ...
      type StrategyParams = { addresses: { [key: string]: number; }; } | { owners?: any; symbol: string; tokens: string[]; weights: number[]; };
        ...
        <div className="flex w-fit flex-col">
          ...
          <div>
            ...
            <label>
              <input
                type="checkbox"
                checked={strategies.some(s => s.name === StrategyName.WHITELIST)}
            ...
            return (
              <div key={index}>
                <input
                  ...
                  type="text"
                  placeholder="Symbol"
                  value={(strategy.params as Erc721MultiRegistryWeightedStrategy['params']).symbol}
                ...
                {erc721Entries.map((entry, entryIndex) => (
                  <div key={entryIndex} className="flex space-x-2">
                    <input
                      ...
                      value={entry.token}
                      onChange={(e) =>
                        handleErc721EntryChange(entryIndex, 'token', e.target.value)
                      }
                    />
                    <input
                      className="text-black"
                      ...
                      value={entry.weight}
                      onChange={(e) =>
                        handleErc721EntryChange(entryIndex, 'weight', +e.target.value)
                      }
                    />
                  </div>
                ))}
