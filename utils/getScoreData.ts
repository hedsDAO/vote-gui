export const getScoreData = (proposal: any) => {
  if (proposal?.scores) {
    ...
    const totalScore = scores.reduce((acc: number, score: number) => acc + score, 0) || 0;
    ...
    const sortedChoicesWithScores = choicesWithScores.sort((a: any, b: any) => b.score - a.score);
    let sortedScores: { [key: string]: any } = {};
    sortedChoicesWithScores.map((choice: any) => {
      sortedScores[choice.id] = { ...choice };
    });
    return { totalScore, sortedChoicesWithScores, sortedScores };
